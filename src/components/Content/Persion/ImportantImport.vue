		...
		<div class="ocontent import ">
			...
			<div v-if="step==1">
				<div class="import-bolck">
					...
					<div class="right ">
						<p class="right-title">填写导入重点人口的信息</p>
						<p class="right-secondtitle">请按照数据模板的格式准备导入数据，模板中的表头名称不可更改，表头行不能删除</p>
						<p class="c-blue"><a class="c-blue" download="人口信息数据录入表格样例.xlsx" :href="this.GLOBAL.loginUrl+'/static/excelTmp/人口信息录入表格样例.xlsx'">下载模板</a></p>
					</div>
				</div>
				<div class="import-bolck">
					<div class="left bg-g"><i class="icon-shangchuan"></i></div>
					<div class="right">
						<p class="right-title">上传填好的重点人口信息表 </p>
						<p class="right-secondtitle">文件后缀名必须为xls 或xlsx （即Excel格式），文件大小不得大于900M</p>

						<el-upload accept=".xls,.xlsx" ref="upload" class="upload-demo" :action="this.GLOBAL.domainUrl+'/fileImportantPopulation/upload'"
			...
			<div v-else class="importComplete">
				...
				<h3>批量导入完成</h3>
				<p>成功导入重点人口数量:{{fileImportRes.insertCount}}条，没能成功导入{{fileImportRes.failCount}}条，更新数据{{fileImportRes.updateCount}}条</p>
...
<script>
	import importForm from '../../component/form/importantImportForm.vue'
	import importHistoryForm from '../../component/form/importantHistory.vue'
	export default {
		data() {
			return {
				step: 1,
		...
		methods: {
			...
			nextStep1() {
				...
				this.$request.post('/fileImportantPopulation/dataImport', {
			...
			getUploadList() {
				var _this = this;
				this.$request.get('/fileImportantPopulation/listNeedImport', {
